// Make sure you're using React 18+ for the useId hook.
import React, { useEffect, useId, useState } from "react";

/**
 * Renders the complex Gemini logo SVG.
 * It uses the React.useId() hook to ensure that SVG element IDs are unique,
 * preventing rendering conflicts when multiple instances are on the same page.
 */
const GeminiLogoSVG = ({ className }: { className: string }) => {
  // Generate a unique base ID for this SVG instance.
  const uniqueId = useId();
  const maskId = `mask_${uniqueId}`;
  const gradientId = `gradient_${uniqueId}`;
  const filterIds = Array.from(
    { length: 11 },
    (_, i) => `filter${i}_${uniqueId}`
  );
...
/**
 * A circular spinner with a Google-style color gradient.
 * It's an SVG circle with a dashed stroke to create the loading effect.
 */
const CircularSpinner = ({ className }: { className: string }) => {
  const uniqueId = useId();
  const spinnerGradientId = `spinner_gradient_${uniqueId}`;
    ...
    >
      <defs>
        ...
        >
          ...
          <stop offset="25%" stopColor="#DB4437" /> {/* Red */}
          <stop offset="50%" stopColor="#F4B400" /> {/* Yellow */}
          <stop offset="75%" stopColor="#0F9D58" /> {/* Green */}
      ...
      <circle
        ...
        strokeWidth="2"
        strokeDasharray="100 25" // Creates a 3/4 circle stroke
        strokeLinecap="round"
        fill="transparent"
      />
    </svg>
...
const LoadingBubble = () => {
  // Use a state variable to track if the spinner should be animating
  ...
  return (
    <div className="w-full flex justify-start">
      <div className="rounded-xl p-2 max-w-max">
        <div className="relative w-10 h-10 flex items-center justify-center">
          {/* Conditionally render the spinner based on the internal state */}
          {isAnimating && (
            <CircularSpinner className="absolute w-full h-full animate-spin" />
          )}
          {/* The Gemini logo is always visible */}
          <GeminiLogoSVG className="w-5 h-5" />
        </div>
      </div>
    </div>
  );
